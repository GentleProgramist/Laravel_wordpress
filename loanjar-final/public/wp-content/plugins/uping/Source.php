...
class Source
{
    ...
    public function toJson()
    {
        if (!is_null($this->logger)) {
            $this->logger->debug("Source::toJson() called");
        }
        $r = $this->validate();
        if ($r === true) {
    ...
    {
        if (!is_null($this->logger)) {
            $this->logger->debug("Source::validate() called");
        }
        ...
        if ($validator->validate()) {
            ...
            if (!is_null($this->logger)) {
             $this->logger->warning("Source validation errors found: ",
    ...
    {
        if (!is_null($this->logger)) {
            $this->logger->debug("Source::getValidationRules() called");
        }
        return [
            'required' => [
                [
    ...
    {
        if (!is_null($this->logger)) {
            $this->logger->debug("Source::getTodayDate() called");
        }
    ...
    {
        if (!is_null($this->logger)) {
            $this->logger->debug("Source::getValidPAYDATE() called");
        }
        $date = new \DateTime("now", new \DateTimeZone("UTC"));
    ...
    {
        if (!is_null($this->logger)) {
            $this->logger->debug("Source::getValidDOB() called");
        }
        $date = new \DateTime("now", new \DateTimeZone("UTC"));
    ...
    {
        if (!is_null($this->logger)) {
            $this->logger->debug("Source::toArray() called");
        }
        $r = $this->validate();
        if ($r === true) {
            return [
...
//            dd('Not Valid');
    ...
    {
        if (!is_null($this->logger)) {
            $this->logger->debug("Source::strDateToJsonDate() called with strdate=$strdate");
        }
//        dd(555);
    ...
    {
        if (!is_null($this->logger)) {
            $this->logger->debug("Source::NormalizePhone() called with phone=$phone and country=$country");
        }
