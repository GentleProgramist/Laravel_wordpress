...
class Helpers {
	 ...
	 */
	public static function download_import_files( $import_file_info, $filter = false ) {
		...
		if($filter && !empty($filter->content) && $filter->content){
			// Check if 'import_file_url' is not defined. That would mean a local file.
		...
		if($filter && !empty($filter->widgets) && $filter->widgets){
		...
		if($filter && !empty($filter->options) && $filter->options){
