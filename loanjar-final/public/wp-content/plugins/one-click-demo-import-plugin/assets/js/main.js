jQuery( function ( $ ) {
	...
	function gridLayoutImport( selectedImportID, $itemContainer ) {
		...
		if( $('.pixfort-import-filter').length ){
			filter.isSet = true;
			if( $('#pixfort_filter_widgets').is(":checked") ){
				filter.widgets = true;
			}
			if( $('#pixfort_filter_content').is(":checked") ){
				filter.content = true;
			}
			if( $('#pixfort_filter_options').is(":checked") ){
				filter.options = true;
			}
		}
	...
	// pixfort import filter
	function displayImportFilter(data){
		var count = 0;
		var output = '';
		// console.log( data );
		var opts = {
			local_import_file: false,
			local_import_redux: false,
			local_import_widget_file: false
		};
		...
		if(count>0){
			output += '<div class="pixfort-import-filter">';
			output += '<div class=""><strong>Import the following data:</strong></div>';

			// output += '<fieldset><legend class="screen-reader-text"><span>All</span></legend><label for="pixfort_filter_all">'+
			// 			'<input name="pixfort_filter_all" type="checkbox" id="pixfort_filter_all" value="">Everyting</label>'+
			// 			'</fieldset>';
						if(data.import_file_url){
							output += '<fieldset><legend class="screen-reader-text"><span>Content</span></legend><label for="pixfort_filter_content">'+
										'<input name="pixfort_filter_content" type="checkbox" id="pixfort_filter_content" value="1" checked="checked">Content</label>'+
										'</fieldset>';
						}else{
							if(data.local_import_file){
								output += '<fieldset><legend class="screen-reader-text"><span>Content</span></legend><label for="pixfort_filter_content">'+
						...
						if(data.import_redux){
							output += '<fieldset><legend class="screen-reader-text"><span>Theme options</span></legend><label for="pixfort_filter_options">'+
										'<input name="pixfort_filter_options" type="checkbox" id="pixfort_filter_options" value="1" checked="checked">Theme options</label>'+
										'</fieldset>';
						}else{
							if(data.local_import_redux){
								output += '<fieldset><legend class="screen-reader-text"><span>Theme options</span></legend><label for="pixfort_filter_options">'+
						...
						if(data.import_widget_file_url){
							output += '<fieldset><legend class="screen-reader-text"><span>Widgets</span></legend><label for="pixfort_filter_widgets">'+
										'<input name="pixfort_filter_widgets" type="checkbox" id="pixfort_filter_widgets" value="1" checked="checked">Widgets</label>'+
										'</fieldset>';
						}else{
							if(data.local_import_widget_file){
								output += '<fieldset><legend class="screen-reader-text"><span>Widgets</span></legend><label for="pixfort_filter_widgets">'+
	...
	function displayConfirmationPopup( selectedImportID, $itemContainer ) {
		var filter = displayImportFilter(ocdi.import_files[ selectedImportID ]);
